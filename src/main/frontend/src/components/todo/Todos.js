...
function Todos({isAuthenticated, setIsAuthenticated}) {
    ...
    useEffect(() => {
        const loadData = async () => {
            let response = null;
            try {
                let url = `http://localhost:3000/api/todo/list?page=${pageNumber - 1}&size=${pageSize}`;

                if(filter === 'Completed'){
                    url = `http://localhost:3000/api/todo/list?page=${pageNumber - 1}${pageSize}?isCompleted=true`;
                } else if(filter === 'Not Completed'){
                    url = `http://localhost:3000/api/todo/list/pages/${pageNumber - 1}/${pageSize}?isCompleted=false`;
                }

                response = await axios.get(url, {headers: {'Authorization': `Bearer ${cookies.token}`,}});
            } catch(error){
                if (error.response) {
    ...
    const markCompleted = async (id) => {
        try {
            await axios.put(`http://localhost:3000/api/todo/${id}/markcomplete`, {}, {
                headers: {
